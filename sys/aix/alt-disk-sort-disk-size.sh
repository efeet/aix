	...
	set -A SELECT_DISKS_OK
	set -A SELECT_DISKS_BIG
	#DISK_SIZE=1024
	#Flag for rootvg
	ROOTVG_COUNT=1
	#Total rootvg disks
	...
	_show_candidates_disks(){
		...
		do
			...
			do
				echo "Candidate Disk:     " ${CANDITATE_DISKS[$(expr $VARSIZE3 - 1)]} " - Size: " ${CANDITATE_DISKS[$VARSIZE3]}
				if [ ${CANDITATE_DISKS[$VARSIZE2]} -eq ${ROOTVG_DISK[$VARSIZE1]} ]
				then
					SELECT_DISKS_OK[$VARSELECTOK]=${CANDITATE_DISKS[$(expr $VARSIZE2 - 1)]}
					VARSELECTOK=$(eval expr $VARSELECTOK + 1)
				else
					if [ ${CANDITATE_DISKS[$VARSIZE3]} -gt ${ROOTVG_DISK[$VARSIZE1]} ] 
					then
						SELECT_DISKS_BIG[$VARSELECTBIG]=${CANDITATE_DISKS[$(expr $VARSIZE2 - 1)]}
						VARSELECTBIG=$(eval expr $VARSELECTBIG + 1)
					fi				
				fi
				VARLOOP2=$(eval expr $VARLOOP2 + 1)
				VARSIZE2=$(eval expr $VARSIZE2 + 2)
				VARSIZE3=$(eval expr $VARSIZE3 + 2)
			done
		...
		echo "------------------------"
		echo  ${#SELECT_DISKS_OK[@]}
		echo ${SELECT_DISKS_OK[@]}
		echo "-----------"
		echo  ${#SELECT_DISKS_BIG[@]}
		echo ${SELECT_DISKS_BIG[@]}
		echo "-----------"
	}
